...
你的任务是从海量论文中筛选出对搜广推有研究价值的论文。

## 判断标准
一篇论文是"相关"的，当它至少满足以下一项：
1. 核心方法可直接应用于推荐系统、广告系统、搜索排序
2. 提出新的召回/粗排/精排/重排算法模型
3. 研究 CTR/CVR/转化率建模、多任务学习、用户行为建模
4. 涉及广告出价、竞价、拍卖机制设计
5. 涉及向量检索、ANN、稠密检索、语义匹配
6. 将 LLM/生成式模型 用于推荐、广告、搜索
7. 研究个性化、用户序列建模、图推荐、强化学习推荐
8. 生成式推荐（Generative Recommendation）相关
9. 传统推荐模型改进（如 FM/DeepFM/DCN 等架构的改进）

## 排除标准
- ❌ 纯工程架构/基础设施优化（如存储、I/O、分布式训练框架）
- ❌ 纯系统性能优化（如 GPU 利用率、编译优化）
- ❌ 与搜索/广告/推荐无关的通用 ML 论文
